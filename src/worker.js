...
function videoRow(row, request){const origin=new URL(request.url).origin;return {...row,video_url:row.video_url||(row.video_key?origin+'/media/'+encodeURIComponent(row.video_key).replace(/%2F/g,'/'):null),thumbnail_url:row.thumbnail_url||(row.thumbnail_key?origin+'/media/'+encodeURIComponent(row.thumbnail_key).replace(/%2F/g,'/'):null)}}
async function listVideos(request,env){if(!env.DB)return json({error:'D1 DB belum terhubung.'},503);const u=new URL(request.url),category=cleanText(u.searchParams.get('category'),50).toLowerCase(),sort=cleanText(u.searchParams.get('sort')||'latest',20),limit=Math.max(1,Math.min(50,Number(u.searchParams.get('limit'))||18));let where="v.status='published' AND (v.published_at IS NULL OR v.published_at<=CURRENT_TIMESTAMP)",bind=[];if(category&&category!=='semua'){where+=' AND c.slug=?';bind.push(category)}const order=sort==='popular'?'v.views_count DESC,v.published_at DESC':sort==='trending'?'v.trending DESC,v.views_count DESC,v.published_at DESC':sort==='recommended'?'v.featured DESC,v.published_at DESC':'v.published_at DESC,v.created_at DESC';const stmt=env.DB.prepare(`SELECT v.*,c.name category,c.slug category_slug,cr.name creator_name,cr.username creator_username,cr.avatar_url creator_avatar FROM videos v LEFT JOIN video_categories c ON c.id=v.category_id LEFT JOIN creators cr ON cr.id=v.creator_id WHERE ${where} ORDER BY ${order} LIMIT ?`).bind(...bind,limit);const data=await stmt.all();return json({videos:(data.results||[]).map(x=>videoRow(x,request)),count:(data.results||[]).length})}
async function getVideo(request,env,id){if(!env.DB)return json({error:'D1 DB belum terhubung.'},503);const row=await env.DB.prepare("SELECT v.*,c.name category,c.slug category_slug,cr.name creator_name,cr.username creator_username,cr.avatar_url creator_avatar FROM videos v LEFT JOIN video_categories c ON c.id=v.category_id LEFT JOIN creators cr ON cr.id=v.creator_id WHERE v.id=? AND v.status='published'").bind(id).first();return row?json(videoRow(row,request)):json({error:'Video tidak ditemukan.'},404)}
async function recordVideoView(request,env,id){if(!env.DB)return json({error:'D1 DB belum terhubung.'},503);let body={};try{body=await request.json()}catch{}const ip=request.headers.get('cf-connecting-ip')||'anon',ua=request.headers.get('user-agent')||'',hashBuf=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(ip+'|'+ua+'|'+id)),viewer=[...new Uint8Array(hashBuf)].slice(0,12).map(x=>x.toString(16).padStart(2,'0')).join('');await env.DB.batch([env.DB.prepare('INSERT INTO video_views(video_id,viewer_hash,watched_seconds,completed) VALUES(?,?,?,?)').bind(id,viewer,Math.max(0,Math.min(86400,Number(body.watched_seconds)||0)),body.completed?1:0),env.DB.prepare('UPDATE videos SET views_count=views_count+1,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(id)]);return json({ok:true})}
async function serveMedia(request,env,key){if(!env.MEDIA)return json({error:'R2 MEDIA belum terhubung.'},503);const obj=await env.MEDIA.get(key);if(!obj)return new Response('Not Found',{status:404});const h=new Headers();obj.writeHttpMetadata(h);h.set('etag',obj.httpEtag);h.set('cache-control','public, max-age=86400');h.set('accept-ranges','bytes');return new Response(obj.body,{headers:h})}
async function uploadVideoMedia(request,env){if(!env.MEDIA)return json({error:'R2 MEDIA belum terhubung.'},503);const type=cleanText(request.headers.get('content-type'),100);if(!type.startsWith('video/')&&!type.startsWith('image/'))return json({error:'Hanya file video atau gambar yang diizinkan.'},415);const size=Number(request.headers.get('content-length')||0);if(size>250*1024*1024)return json({error:'Ukuran file maksimal 250 MB untuk upload langsung.'},413);const ext=(type.split('/')[1]||'bin').replace(/[^a-z0-9]/gi,'').slice(0,8),kind=type.startsWith('video/')?'videos':'thumbnails',key=kind+'/'+new Date().toISOString().slice(0,7)+'/'+crypto.randomUUID()+'.'+ext;await env.MEDIA.put(key,request.body,{httpMetadata:{contentType:type}});return json({ok:true,key,url:'/media/'+key},201)}
async function createVideo(request,env){if(!env.DB)return json({error:'D1 DB belum terhubung.'},503);let b;try{b=await request.json()}catch{return json({error:'Body JSON tidak valid.'},400)}const title=cleanText(b.title,180),videoKey=cleanText(b.video_key,500);if(!title||!videoKey)return json({error:'title dan video_key wajib.'},400);const slug=(cleanText(b.slug||title,180).toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||crypto.randomUUID());const cat=b.category_slug?await env.DB.prepare('SELECT id FROM video_categories WHERE slug=?').bind(cleanText(b.category_slug,50)).first():null;const r=await env.DB.prepare("INSERT INTO videos(title,slug,description,category_id,video_key,thumbnail_key,duration_seconds,status,featured,trending,published_at) VALUES(?,?,?,?,?,?,?,?,?,?,CASE WHEN ?='published' THEN CURRENT_TIMESTAMP ELSE NULL END)").bind(title,slug,cleanText(b.description,5000),cat?.id||null,videoKey,cleanText(b.thumbnail_key,500)||null,Math.max(0,Number(b.duration_seconds)||0),['draft','review','scheduled','published','archived'].includes(b.status)?b.status:'draft',b.featured?1:0,b.trending?1:0,['draft','review','scheduled','published','archived'].includes(b.status)?b.status:'draft').run();return json({ok:true,id:r.meta?.last_row_id,slug},201)}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/videos' && request.method === 'GET') return listVideos(request, env);
      const videoMatch=url.pathname.match(/^\/api\/videos\/(\d+)$/);if(videoMatch&&request.method==='GET')return getVideo(request,env,Number(videoMatch[1]));
      const viewMatch=url.pathname.match(/^\/api\/videos\/(\d+)\/view$/);if(viewMatch&&request.method==='POST')return recordVideoView(request,env,Number(viewMatch[1]));
      if(url.pathname.startsWith('/media/')&&request.method==='GET')return serveMedia(request,env,decodeURIComponent(url.pathname.slice(7)));
      if(url.pathname==='/api/admin/media/upload'&&request.method==='POST'){if(!isAdminHost||accessEmail(request)!==adminEmail(env))return forbiddenAdmin();return uploadVideoMedia(request,env)}
      if(url.pathname==='/api/admin/videos'&&request.method==='POST'){if(!isAdminHost||accessEmail(request)!==adminEmail(env))return forbiddenAdmin();return createVideo(request,env)}
      ...
      if (url.pathname === '/api/health') {
        const health = json({ ok: true, app: 'PURBALINK', version: '6.2.0', environment: host.endsWith('.workers.dev') ? 'preview' : 'production', public_host: PUBLIC_HOST, admin_host: ADMIN_HOST, payment_gateway: 'Midtrans', mode: mode(env), midtrans_configured: Boolean(env.MIDTRANS_SERVER_KEY), jooble_configured: Boolean(env.JOOBLE_API_KEY), d1_configured:Boolean(env.DB), r2_configured:Boolean(env.MEDIA), ai: { openai:Boolean(env.OPENAI_API_KEY), gemini:Boolean(env.GEMINI_API_KEY), deepseek:Boolean(env.DEEPSEEK_API_KEY), groq:Boolean(env.GROQ_API_KEY), mistral:Boolean(env.MISTRAL_API_KEY), anthropic:Boolean(env.ANTHROPIC_API_KEY), openrouter:Boolean(env.OPENROUTER_API_KEY), together:Boolean(env.TOGETHER_API_KEY) } });
