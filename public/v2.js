(()=>{
  ...
  function initHome(){
    ...
    if(typeof window.shareArticle==='function'){window.shareArticle=function(){const aId=new URLSearchParams(location.search).get('article')||1;const a=db.articles.find(x=>String(x.id)===String(aId))||db.articles[0];const preview=String(a.summary||a.body||'').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim().slice(0,220);const u=new URL(DOMAIN+'/');u.searchParams.set('article',a.id);u.searchParams.set('share_title',a.title);u.searchParams.set('share_desc',preview);if(a.img)u.searchParams.set('share_image',a.img);const url=u.toString();const text=preview?preview.slice(0,120):'Baca selengkapnya di PURBALINK';if(navigator.share)navigator.share({title:`${a.title} — PURBALINK`,text,url}).catch(()=>{});else navigator.clipboard?.writeText(url).then(()=>toast('Tautan artikel dengan preview disalin'))}}
