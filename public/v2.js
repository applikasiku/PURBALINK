(()=>{
  ...
  function initShop(){
    ...
    function openProduct(id){
      ...
      <section class="pd-community"><div class="pd-summary"><div><b>${baseRating}</b><span>★★★★★</span><small>${ratingCount} rating produk</small></div><div><strong>Ulasan & Diskusi</strong><small>Pengalaman pembeli dan tanya jawab produk</small></div></div><div class="pd-tabs"><button class="active" data-pd-tab="reviews">Ulasan (${reviews.length})</button><button data-pd-tab="discussions">Diskusi (${discussions.length})</button></div><div id="pdSocialContent" data-tab="reviews"></div><div class="pd-composer"><textarea id="pdComposerText" placeholder="Tulis ulasan atau pertanyaan tentang produk…"></textarea><div class="pd-composer-tools"><button type="button" data-insert="😀" aria-label="Senang"><img src="https://img.icons8.com/emoji/48/grinning-face.png" alt=""></button><button type="button" data-insert="👍" aria-label="Suka"><img src="https://img.icons8.com/emoji/48/thumbs-up.png" alt=""></button><button type="button" data-insert="❤️" aria-label="Cinta"><img src="https://img.icons8.com/emoji/48/red-heart.png" alt=""></button><button type="button" data-insert="😂" aria-label="Lucu"><img src="https://img.icons8.com/emoji/48/face-with-tears-of-joy.png" alt=""></button><button type="button" data-insert="🔥" aria-label="Mantap"><img src="https://img.icons8.com/emoji/48/fire.png" alt=""></button><label class="pd-media-btn">GIF/Sticker<input id="pdMediaUrl" placeholder="URL GIF/sticker"></label><button class="pd-send" id="pdSend">Kirim</button></div></div></section>
      ...
      document.querySelectorAll('[data-insert]').forEach(btn=>btn.onclick=()=>{const t=document.getElementById('pdComposerText');t.value+=(t.value?' ':'')+btn.dataset.insert;t.focus()});
      document.getElementById('pdSend').onclick=()=>{const t=document.getElementById('pdComposerText'),media=document.getElementById('pdMediaUrl'),text=t.value.trim();if(!text&&!media.value.trim()){toast('Tulis pesan atau masukkan GIF/sticker');return}const item={name:currentUser()?.name||'Pengguna PURBALINK',text,media:media.value.trim(),date:new Date().toLocaleDateString('id-ID'),likes:0};if(document.getElementById('pdSocialContent').dataset.tab==='reviews'){reviews.unshift(item);setLocal(reviewKey,reviews)}else{discussions.unshift(item);setLocal(discussionKey,discussions)}t.value='';media.value='';renderSocial();toast('Berhasil dikirim')};renderSocial();
    }
