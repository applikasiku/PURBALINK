...
async function checkIntegrations(){
 var cards=[].slice.call(document.querySelectorAll('[data-int]'));
 cards.forEach(function(c){c.querySelector('.int-dot').className='int-dot warn';c.querySelector('b').textContent='Mengecek…'});
 try{
  var res=await fetch('/api/health',{cache:'no-store',signal:AbortSignal.timeout(10000)});
  if(!res.ok)throw new Error('HTTP '+res.status);
  var health=await res.json();
  cards.forEach(function(c){
   var name=c.dataset.int,ready=name==='Cloudflare Worker'?health.ok:name==='Jooble'?health.jooble_configured:name==='Midtrans'?health.midtrans_configured:Object.values(health.ai||{}).some(Boolean);
   c.querySelector('.int-dot').className='int-dot '+(ready?'ok':'warn');
   c.querySelector('b').textContent=name==='Cloudflare Worker'?(ready?'Terhubung · '+health.version:'Tidak tersedia'):(ready?'Dikonfigurasi · belum diuji':'Belum dikonfigurasi');
  });
 }catch(e){cards.forEach(function(c){c.querySelector('.int-dot').className='int-dot bad';c.querySelector('b').textContent='Pemeriksaan gagal. Coba lagi.'})}
}
